...
module.exports = function(taskName, userConfig){
  ...
  if (!userConfig.server && !localConfig.hostname){
    quench.logError(
      "Browser-sync requires a \`server\` path in userConfig, or a `hostname` defined in local.js!\n",
  ...
  const browserSyncSettings = R.merge({
    ...
    proxy: (localConfig.hostname) || undefined
